...
if C["talent"].SESEnable ~= true then return end
...
-- Gear Settings
local Enablegear = C["talent"].SESEnableGear -- herp
local Autogearswap = C["talent"].SESGearSwap -- derp
local set1 = C["talent"].SESSet1 -- this is the gear set that gets equiped with your primary spec. (must be the NUMBER from 1-10)
local set2 = C["talent"].SESSet2 -- this is the gear set that gets equiped with your secondary spec.(must be the NUMBER from 1-10)
...
spec.t:SetFont(C["medias"].Font, 12, "THINOUTLINE")
...
D.CreateBtn("MB_reload", UIParent, 19, 19, "ReloadUI", "R")
...
toggle.t:SetFont(C["medias"].Font, 12)
