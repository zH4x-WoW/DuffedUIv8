...
do
	...
	local CheckFilter = function(self, id, caster, filter)
		...
		for _, v in ipairs(filter) do if (v.id == id and (v.castByAnyone or byPlayer)) then return v end end
...
do
	...
	do
		...
		local SetCastSpellId = function(self, id) self.castSpellId = id end
		...
		CreateAuraBar = function(parent)
			...
			stacks:SetFont(C["medias"].Font, 12, "THINOUTLINE")
	...
	local function SetHiddenHeight(self, height) self.hiddenHeight = height end
	...
	CreateAuraBarFrame = function(dataSource, parent)
		...
		border:SetBackdrop {
		  edgeFile = C["medias"].Blank, edgeSize = 1,
		  insets = {left = 0, right = 0, top = 0, bottom = 0}
		}
		border:SetBackdropColor(0, 0, 0, 0)
		border:SetBackdropBorderColor(unpack(C["general"].BackdropColor))
