		...
		if (unit == "player") then
			...
			if C["unitframes"].classbar then
				if D.myclass == "MAGE" then
					...
					if C["unitframes"].runeofpower then
						...
						if C["unitframes"].movableclassbar then
							...
							rp:Point("TOP", power, "BOTTOM", -53, -12)
						end
						rp:SetWidth(100)
						rp:SetHeight(5)
				...
				if D.myclass == "DEATHKNIGHT" then
					...
					Runes:Size(100, 5)
					if C["unitframes"].movableclassbar then
						Runes:Point("BOTTOM", CBAnchor, "TOP", -Runes:GetWidth() / 2, 0)
					else
				...
				if D.myclass == "SHAMAN" then
					...
					for i = 1, 4 do
						...
						if i == 1 then
							if C["unitframes"].movableclassbar then
								TotemBar:Point("BOTTOM", CBAnchor, "TOP", 0, 0)
							else
								TotemBar[i]:Point("TOP", power, "BOTTOM", -84, -1)
							end
						else
