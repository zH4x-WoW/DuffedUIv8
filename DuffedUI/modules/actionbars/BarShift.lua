		...
		for i = 1, NUM_STANCE_SLOTS do
			...
			if i ~= 1 then
				...
				if C['actionbar']['verticalshapeshift'] then
					button:Point('TOP', previous, 'BOTTOM', 0, -space)
				else
					button:Point('LEFT', previous, 'RIGHT', space, 0)
				end
			end
...
if C['actionbar']['verticalshapeshift'] then
	ssborder:Point('TOP', 0, space)
else
	ssborder:Point('LEFT', -space, 0)
end
if not C['actionbar']['shapeshiftborder'] then
	ssborder:SetAlpha(0)
else
