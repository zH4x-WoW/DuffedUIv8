...
local mediaPath = [[Interface\AddOns\DuffedUI\media\textures\]]
local Textures = {
...
function MyButton:OnAdd()
	self:SetScript('OnMouseUp', function(self, mouseButton)
		if (mouseButton == 'RightButton') and (IsAltKeyDown()) and (IsControlKeyDown()) then
			local _, tID, f
			--adjust values when the Molinari AddOn is active
			if self:GetName() == "Molinari" then
				_, f = self:GetPoint()
				tID = GetContainerItemID(f.bagID, f.slotID)
			else
				f = self
				tID = GetContainerItemID(self.bagID, self.slotID)
			end
			if tID then 
				cbNivCatDropDown.itemName = GetItemInfo(tID)
				cbNivCatDropDown.itemID = tID
				--ToggleDropDownMenu(1, nil, cbNivCatDropDown, self, 0, 0)
				cbNivCatDropDown:Toggle(f, nil, nil, 0, 0)
			end
		end
	end)
end
