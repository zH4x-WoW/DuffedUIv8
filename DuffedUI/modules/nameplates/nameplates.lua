...
local function CreateAuraIcon(parent)
	...
	button.count:SetTextColor(.85, .89, .25)
...
local function UpdateAuraIcon(button, unit, index, filter)
	...
	button.cd:SetScript("OnUpdate", function(self)
		...
		button.cd.timer.text:SetTextColor(.85, .89, .25)
		button.cd.timer.text:SetPoint("TOP", button, "TOP", 1, 3)
	end)
	button:Show()
end
...
local function OnAura(frame, unit)
	...
	local i = 1
	for index = 1, 40 do
		if i > C["nameplate"].width / C["nameplate"].auraswidth then return end
		...
		if duration and match == true then
			...
			if i ~= 1 and i <= C["nameplate"].width / C["nameplate"].auraswidth then
